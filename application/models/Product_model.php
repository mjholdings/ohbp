...
class Product_model extends MY_Model {
    ...
    // Lấy chi tiết thông tin user hiện tại
    ...
    function getCurrencyHtml($control = 'admincontrol', $theme = null, $onlyList = null) {
        ...
        if ($theme == 'store-default') {
            ...
            aria-expanded="false">
            <span class="currency-code bg-secondary text-white rounded-circle" style="font-size:10px;display: inline-block; width: 33px; height: 33px; line-height: 33px; text-align: center;">' . $selected['code'] . '</span>
            ' . $caretIcon . '
            </a>';
    ...
    function getPopulerUsers($filter = array(), $popular_aff_filter = '') {
        ...
        $arrChildrenIds = $this->getAllChildrenUserId($filter['current_user_id']);
        $allChildrenIds = $this->getAllIdsRecursive($arrChildrenIds);
        $arrChildrenId = [$filter['current_user_id']];
        foreach ($allChildrenIds as $children) {
            array_push($arrChildrenId, $children);
        }

        $query = 'SELECT sum(wallet.amount) as amount,sum(wallet.amount) as all_commition, wallet.user_id, users.type, users.avatar, users.firstname, users.lastname, users.Country, users.email, countries.sortname, users.user_star FROM wallet LEFT JOIN users ON wallet.user_id = users.id LEFT JOIN countries ON countries.id = users.Country WHERE wallet.status > 0 AND users.id IN (' . implode(",", $arrChildrenId) . ') AND wallet.withdraw_request = 0 AND wallet.type NOT IN("vendor_sale_commission") AND users.type = "user" ' . $where . ' GROUP BY wallet.user_id ORDER BY amount DESC ' . $limit;
    ...
    public function getAllUsersTreeV3($filter = array(), $user_id = 0, $first_time = true, $is_admin = false) {
        ...
        if ($this->level_count >= $this->max_level)
            return [];
    ...
    public function getAllUsersTreeV2ForUser($user_id, $first_time = true) {
        ...
        if ($this->level_count >= ($max_level) + 2)
            return array();
    ...
    function getAllIdsRecursive($data) {
        ...
        foreach ($data as $item) {
            ...
            if (isset($item['children']) && is_array($item['children']) && !empty($item['children'])) {
                $ids = array_merge($ids, $this->getAllIdsRecursive($item['children']));
            }
        }
    ...
    public function getAllChildrenUserId($parent) {
        ...
        $result = $this->db->query("SELECT id,refid FROM users WHERE type='user' AND refid = {$parent}")->result_array();

        foreach ($result as $value) {
            $c = $this->getAllChildrenUserId($value['id']);
    ...
    public function getAllUserIdInBranch($child) {
        ...
        if ($result->refid > 1) {
            return $this->getAllUserIdInBranch($result->refid);
        } else {
            $arrChildrenUserId = $this->getAllChildrenUserId($result->id);
            return $this->getAllIdsRecursive($arrChildrenUserId);
        }
    }
    ...
    function setFormClicks($form_id, $user_id, $viewer_id = 0) {
        ...
        if ($result == 0) {
            ...
            if ($viewer_id)
                $new_record['viewer_id'] = $viewer_id;
    ...
    public function referClick($product, $user_id, $viewer_id = 0, $wallet_group_id = null) {
        ...
        if ((int) $store_commition_setting['status'] == 0) {
            return false;
        } else if ((int) $store_commition_setting['status'] == 2 && in_array($user_id, $disabled_for)) {
            return false;
        }
    ...
    public function getMyLevel($user_id, $setting = null) {
        ...
        $q = $this->db->query(
            "SELECT " . implode(",", $select) . ", l0.id as myid
        FROM `users` as l0
        " . implode(" ", $join) . "
    ...
    private function get_types($filetype) {
        switch (true) {
            case preg_match('/image/', $filetype):
                return 'image';
            case preg_match('/video/', $filetype):
                return 'video';
            case preg_match('/audio/', $filetype):
                return 'audio';
            case preg_match('/pdf/', $filetype):
                return 'pdf';
            case preg_match('/(csv|excel)/', $filetype):
                return 'spreadsheet';
            case preg_match('/powerpoint/', $filetype):
                return 'powerpoint';
            case preg_match('/(msword|text)/', $filetype):
                return 'document';
            case preg_match('/zip/', $filetype):
                return 'zip';
            case preg_match('/rar/', $filetype):
                return 'rar';
            default:
