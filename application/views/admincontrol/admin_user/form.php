				...
				<form id="admin-form">
					...
					<div class="row">
						<div class="col-md-4 mb-3">
							<div class="form-floating">
								<select name="role" class="form-control countries" id="role">
									<option value="" selected="selected"><?= __('Chọn vai trò') ?></option>
									<?php foreach ($role as $role_item) : ?>
										<option <?php if (!empty($user->role) && $user->role == $role_item->role) { ?> selected <?php } ?> value="<?php echo $role_item->role; ?>"><?php echo $role_item->name; ?></option>
									<?php endforeach; ?>
								</select>
								<label for="role"><?= __('Vai trò') ?></label>
							</div>
						</div>
						<div class="col-md-4 mb-3">
							<div class="form-floating">
