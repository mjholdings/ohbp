		...
		<tr>
			...
			<td>
				<?php if($review['rating_created_by'] == $user_id) { ?>
					<a href="<?= base_url('admincontrol/manage_review/'.$review['rating_id'])  ?>" class="btn btn-primary edit-button"><?= __("admin.edit") ?></a>
				<?php } else { ?>
					<a href="#" class="btn btn-primary edit-button disabled"  ><?= __("admin.edit") ?></a>
				<?php } ?>
				<a href="<?= base_url('admincontrol/deleteReview/'.$review['rating_id']);?>" class="btn btn-danger delete-button" onclick="return onDeleteReview(<?= $review['rating_id'] ?>);" ><?= __("admin.delete") ?></a>
			</td>
		</tr>
	<?php } 
} ?>
