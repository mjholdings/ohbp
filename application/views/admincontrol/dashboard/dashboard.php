...
<!--                            </li>-->
                        ...
                        <div class="tab-content">
                                            ...
                                            <div class="card bg-light position-relative">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <div class="row justify-content-center mb-2">
                                                        <div class="col text-center">
                                                            <span class="card-text h5">
                                                                <span class="card-text h5 ajax-click_action_total"><?= (int)$admin_totals['click_action_total'] ?></span>
                                                                <span class="mx-1">/</span>
                                                                <span class="card-text h5 ajax-click_action_commission"><?= $fun_c_format($admin_totals['click_action_commission']) ?></span>
                                                            </span>
                                                        </div>
                                                    </div>
                                                    <!-- Row 3: Second Amount -->
                                            ...
                                            <div class="card bg-light position-relative">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <div class="row justify-content-center mb-2">
                                                        <div class="col text-center">
                                                            <span class="card-text h5">
                                                                <span class="card-text h5 ajax-all_click_total"><?= (int)($admin_totals['click_localstore_total'] + $admin_totals['click_integration_total'] + $admin_totals['click_form_total']) ?></span>
                                                                <span class="mx-1">/</span>
                                                                <span class="card-text h5 ajax-all_click_commission"><?= $fun_c_format($admin_totals['click_localstore_commission'] + $admin_totals['click_integration_commission'] + $admin_totals['click_form_commission']) ?></span>
                                                            </span>
                                                        </div>
                                                    </div>
                                                    <!-- Row 3: Second Amount -->
                            ...
                            <div class="tab-pane fade" id="settings" role="tabpanel" aria-labelledby="settings-tab">
                                ...
                                <div class="row mt-4">
                                    <label>
                                        <div class="mb-1"><strong><?= __('admin.admin_mode_explanation_1') ?></strong>
                                        </div>
                                        <ul>
                                            <li>
                                                <span class="fw-bold"><?= __('admin.local_store_mode') ?>:</span> <?= __('admin.local_store_mode_description') ?>
                                            </li>
                                            <li>
                                                <span class="fw-bold"><?= __('admin.external_mode') ?>:</span> <?= __('admin.external_mode_description') ?>
                                            </li>
                                        </ul>
                                        <p>
                                            <strong><?= __('admin.note') ?>
                                                :</strong> <?= __('admin.admin_mode_explanation_2') ?>
                                        </p>
                                    </label>
                                </div>
                                <!-- Mode Settings -->
...
foreach ($integration_logs as $key => $log) {
    if ($last_id_integration_logs <= $log['id']) {
        $last_id_integration_logs = $log['id'];
    }
}
foreach ($integration_orders as $key => $order) {
    if ($last_id_integration_orders <= $order['id']) {
        $last_id_integration_orders = $order['id'];
    }
}
foreach ($store_orders as $key => $order) {
    if ($last_id_store_orders <= $order['id']) {
        $last_id_store_orders = $order['id'];
    }
}
foreach ($newuser as $users) {
    if ($last_id_newuser <= $users['id']) {
        $last_id_newuser = $users['id'];
    }
}
